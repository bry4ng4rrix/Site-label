  ...
  return (
    <section className="py-20 px-6" style={{ backgroundColor: "var(--paper)" }}>
      <div className="max-w-7xl mx-auto">
        <p className="label-tag mb-10 text-center" style={{ color: "var(--brand)" }}>ILS NOUS FONT CONFIANCE</p>
        <div className="grid md:grid-cols-3 gap-px bg-black/8">
          {items.map((t, i) => (
            <div key={i} className="p-8 flex flex-col gap-4" style={{ backgroundColor: "var(--white)" }}>
              <span className="font-display text-4xl leading-none" style={{ color: "var(--brand)" }}>"</span>
              ...
              <div className="pt-4 border-t" style={{ borderColor: "rgba(0,0,0,0.08)" }}>
                ...
                <p className="text-xs font-light" style={{ color: "var(--mid)" }}>{t.role} — {t.company}</p>
              </div>
            </div>
          ))}
        </div>
